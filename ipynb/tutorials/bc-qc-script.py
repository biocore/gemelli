...
bulk (see above) w/ qc_rarefaction for BC 
...
for study_, study_mf_ in bulk_studies_mf.groupby('qiita_study_id'):
    ...
    # non-rare run before
    dist_norare = beta_diversity('braycurtis', bt_study_.matrix_data.toarray().T, ids=bt_study_.ids())
    ...
    for fold_rare in range(n_iter_subsample):
        ...
        dist_rare = beta_diversity('braycurtis', bt_study_.copy().subsample(rare_level).matrix_data.toarray().T, ids=bt_study_.ids())
...
qc_results_testing_df.to_csv('bc-qc-rare-results-all.csv')